...
export function buildHomeText({ cfg, menuDivider, supportUsername }) {
  return [
    ...
    '4. Start, refresh, monitor, and manage everything from Telegram.',
    '',
    menuDivider,
    ...
    `\u{1F4B8} Built-in handled trading routes use *${formatBpsPercent(cfg.tradingHandlingFeeBps)}* per trade, which is roughly half the 1% fee many major Telegram bots advertise`,
...
export function buildHelpText({ cfg, menuDivider, supportUsername }) {
  return [
    ...
    '4. Start the automation and use Refresh to see the latest status, balances, and stats.',
    '',
    menuDivider,
